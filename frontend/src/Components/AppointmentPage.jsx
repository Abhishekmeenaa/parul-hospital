...
const AppointmentPage = () => {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState(
    doctorId ? featuredDoctors.find(doc => doc.id === parseInt(doctorId)) : null
  );
...
const generateTimeSlots = () => {
  ...
  selectedDoctor.timings.forEach(timing => {
    ...
    while (currentHour < end.hours || (currentHour === end.hours && currentMinute < end.minutes)) {
      ...
      allSlots.push({
        value: `${currentHour.toString().padStart(2, '0')}:${currentMinute.toString().padStart(2, '0')}`,
        label: `${displayHour}:${currentMinute.toString().padStart(2, '0')} ${period} (${timing.day})`
      });
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6">
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="bg-[#00A297] text-white p-4 md:p-6">
          ...
          {selectedDoctor && <p className="opacity-90">{selectedDoctor.specialization}</p>}
        </div>

        <div className={`grid ${selectedDoctor ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-8 p-4 md:p-6`}>
          {selectedDoctor && (
            <div className="space-y-6">
              <div className="flex items-start">
                <div className="bg-[#E8F5F4] p-3 rounded-full mr-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">{selectedDoctor.qualification}</p>
                </div>
              </div>
                ...
                {!doctorId && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const selected = featuredDoctors.find(doc => doc.id === parseInt(e.target.value));
